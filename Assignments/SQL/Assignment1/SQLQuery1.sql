...
create table Employees (Empno int PRIMARY KEY,Ename varchar(20) NOT NULL,Job varchar(15),Salary decimal(7,2) CHECK (Salary > 0),Deptno int,
CONSTRAINT fk_emp_dept FOREIGN KEY (Deptno) REFERENCES Departments(Deptno));


--creating projects table
create table Projects (Project_ID int PRIMARY KEY,Descr varchar(50) NOT NULL,Start_Date DATE,Planned_End_Date DATE,Actual_End_Date DATE,Budget decimal(10,2) CHECK (Budget > 0),Client_ID int,
CONSTRAINT fk_proj_client FOREIGN KEY (Client_ID) REFERENCES Clients(Client_ID),
CONSTRAINT chk_dates CHECK (Actual_End_Date IS NULL OR Actual_End_Date > Planned_End_Date));


--creating empproject tasks table
create table EmpProjectTasks (Project_ID int,Empno int,Start_Date DATE,End_Date DATE,Task varchar(25) NOT NULL,Status varchar(15) NOT NULL,CONSTRAINT pk_ept PRIMARY KEY (Project_ID, Empno),
CONSTRAINT fk_ept_proj FOREIGN KEY (Project_ID) REFERENCES Projects(Project_ID),
CONSTRAINT fk_ept_emp FOREIGN KEY (Empno) REFERENCES Employees(Empno));
